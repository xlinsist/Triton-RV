...
def plot_all_benchmarks_bar_chart(input_csv):
    ...
    # 筛选我们感兴趣的benchmarks
    df = df[df["benchmark"].isin(["matmul", "layernorm", "correlation", "dropout", "softmax", "resize", "rope"])]
...
def get_best_thread(input_csv, output_csv):
    ...
    for benchmark in unique_benchmarks:
        ...
        for shape in unique_shapes:
            ...
            # Measure t4_best_param under T1, T8, and t4 threads to find the optimal result
            p2_speedup = 0
            for thread in [1, 4, 8]:
                thread_row = sub_df[(sub_df['thread'] == thread) & (sub_df['parameter'] == t8_best_param)]
                if not thread_row.empty:
                    p2_speedup = max(p2_speedup, thread_row['speedup'].max())
            # for strategy, speedup in [("clang", 1), ("triton", t8_best_speedup), ("OUR", our_speedup)]:
